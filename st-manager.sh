...
fetch_remote_versions() {
    ...
    # 修复：使用 GitHub API 获取最新的 Release Tag，这是最准确的方法
    ...
    if [ $? -eq 0 ] && [ -n "$release_json" ]; then
        RELEASE_VER=$(echo "$release_json" | grep '"tag_name":' | sed 's/.*: "\(.*\)".*/\1/')
    else
        RELEASE_VER="查询失败"
    fi

    # Staging 版本仍然从其分支的 package.json 获取
