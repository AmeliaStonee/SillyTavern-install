...
run_backup() {
    ...
    find "${BACKUP_DIR}" -name "SillyTavern-Backup-*.zip" -mtime +"${BACKUP_DAYS}" -exec rm {} \;
    log "清理完成。"
    return 0
}
...
update_self() {
    ...
    if cmp -s "${SCRIPT_PATH}" "${temp_file}"; then
        ...
        echo -e "${C_GREEN}发现新版本！正在更新...${C_RESET}"
...
setup_script() {
    ...
    echo -e "${C_GREEN}设置完成！正在进入主菜单...${C_RESET}"
    sleep 2
    exec bash "${SCRIPT_PATH}"
}
...
main_menu() {
    while true; do
        ...
        echo -e "  ${C_GREEN}1. ${main_option_text}${C_RESET} (默认选项, 直接回车)"
        ...
        case $choice in
            1) 
                if $is_installed; then start_sillytavern; else install_sillytavern; fi ;;
            2) 
                if $is_installed; then update_sillytavern; else echo -e "${C_RED}请先安装 SillyTavern (选项1)。${C_RESET}"; sleep 1; fi ;;
            3) 
                setup_backup; read -p "按回车键返回主菜单..." ;;
            4) 
                if [ -f "${CONFIG_FILE}" ]; then manage_backup; else echo -e "${C_RED}请先配置备份功能 (选项3)。${C_RESET}"; sleep 1; fi ;;
            5) 
                manual_backup ;;
            6) 
                update_self ;;
