...
fetch_remote_versions() {
    ...
    RELEASE_VER=$(wget -qO- --timeout=5 "$release_url" | grep '"version":' | sed 's/.*: "\(.*\)".*/\1/')
    STAGING_VER=$(wget -qO- --timeout=5 "$staging_url" | grep '"version":' | sed 's/.*: "\(.*\)".*/\1/')

    [ -z "$RELEASE_VER" ] && RELEASE_VER="查询失败"
    [ -z "$STAGING_VER" ] && STAGING_VER="查询失败"
}
...
run_backup() { load_backup_config; if [ -z "${BACKUP_DIR}" ] || [ -z "${BACKUP_DAYS}" ]; then log "备份失败：未配置"; return 1; fi; local data_dir="${ST_DIR}/${ST_DATA_USER_SUBPATH}"; if [ ! -d "${data_dir}" ]; then log "备份失败：找不到数据目录 ${data_dir}"; return 1; fi; mkdir -p "${BACKUP_DIR}"; local timestamp=$(date +%Y%m%d-%H%M); local filename="SillyTavern-Backup-${timestamp}.zip"; log "开始备份到 ${BACKUP_DIR}/${filename}"; local parent_dir=$(dirname "${data_dir}"); local user_data_folder=$(basename "${data_dir}"); pushd "${parent_dir}" > /dev/null; zip -r "${BACKUP_DIR}/${filename}" "${user_data_folder}"; local zip_status=$?; popd > /dev/null; if [ ${zip_status} -eq 0 ]; then log "备份成功"; else log "备份失败：zip出错"; return 1; fi; log "清理旧备份..."; find "${BACKUP_DIR}" -name "SillyTavern-Backup-*.zip" -mtime +"${BACKUP_DAYS}" -exec rm {} \;; log "清理完成"; return 0; }
setup_backup() { echo -e "${C_CYAN}=== 配置自动备份 ===${C_RESET}"; local default_backup_dir="${HOME}/SillyTavern-Backups"; read -p "请输入备份目录 (默认: ${default_backup_dir}): " backup_dir; BACKUP_DIR=${backup_dir:-${default_backup_dir}}; read -p "保留最近多少天的备份？ (默认: 7): " backup_days; BACKUP_DAYS=${backup_days:-7}; local hour minute; while true; do read -p "设置备份小时 (0-23): " hour; if [[ "$hour" =~ ^[0-9]+$ ]] && [ "$hour" -ge 0 ] && [ "$hour" -le 23 ]; then break; else echo -e "${C_RED}输入无效。${C_RESET}"; fi; done; while true; do read -p "设置备份分钟 (0-59): " minute; if [[ "$minute" =~ ^[0-9]+$ ]] && [ "$minute" -ge 0 ] && [ "$minute" -le 59 ]; then break; else echo -e "${C_RED}输入无效。${C_RESET}"; fi; done; BACKUP_TIME=$(printf "%02d:%02d" "$hour" "$minute"); mkdir -p "${SCRIPT_INSTALL_DIR}"; echo "BACKUP_DIR=\"${BACKUP_DIR}\"" > "${CONFIG_FILE}"; echo "BACKUP_DAYS=\"${BACKUP_DAYS}\"" >> "${CONFIG_FILE}"; echo "BACKUP_TIME=\"${BACKUP_TIME}\"" >> "${CONFIG_FILE}"; echo -e "${C_GREEN}配置已保存。${C_RESET}"; install_cron_job; }
install_cron_job() { load_backup_config; if [ -z "${BACKUP_TIME}" ]; then echo -e "${C_RED}错误: 备份时间未设置。${C_RESET}"; return; fi; local minute=$(echo "$BACKUP_TIME" | cut -d: -f2); local hour=$(echo "$BACKUP_TIME" | cut -d: -f1); if ! command -v crond &> /dev/null; then pkg install cronie -y; fi; local cron_job="${minute} ${hour} * * * ${SCRIPT_PATH} --run-backup"; (crontab -l 2>/dev/null | grep -v "${SCRIPT_PATH}") | { cat; echo "${cron_job}"; } | crontab -; echo -e "${C_GREEN}自动备份任务已设置为每天 ${BACKUP_TIME} 执行。${C_RESET}"; if ! pgrep -f "crond" > /dev/null; then crond; echo -e "${C_GREEN}定时服务 (crond) 已启动。${C_RESET}"; fi; }
manage_backup() {
    while true; do
        ...
        choice=${choice:-0}
        case $choice in
            1) local hour minute; while true; do read -p "输入新小时 (0-23): " hour; if [[ "$hour" =~ ^[0-9]+$ ]] && [ "$hour" -ge 0 ] && [ "$hour" -le 23 ]; then break; else echo -e "${C_RED}无效。${C_RESET}"; fi; done; while true; do read -p "输入新分钟 (0-59): " minute; if [[ "$minute" =~ ^[0-9]+$ ]] && [ "$minute" -ge 0 ] && [ "$minute" -le 59 ]; then break; else echo -e "${C_RED}无效。${C_RESET}"; fi; done; local new_time=$(printf "%02d:%02d" "$hour" "$minute"); sed -i "s|BACKUP_TIME=.*|BACKUP_TIME=\"${new_time}\"|" "${CONFIG_FILE}"; install_cron_job; echo -e "${C_GREEN}时间已更新。${C_RESET}"; sleep 1 ;;
            2) read -p "输入新保留天数: " new_days; if [[ "$new_days" =~ ^[0-9]+$ ]]; then sed -i "s|BACKUP_DAYS=.*|BACKUP_DAYS=\"${new_days}\"|" "${CONFIG_FILE}" && echo -e "${C_GREEN}已更新。${C_RESET}"; else echo -e "${C_RED}请输入数字。${C_RESET}"; fi; sleep 1 ;;
            3) read -p "输入新备份目录: " new_dir; sed -i "s|BACKUP_DIR=.*|BACKUP_DIR=\"${new_dir}\"|" "${CONFIG_FILE}" && echo -e "${C_GREEN}已更新。${C_RESET}"; sleep 1 ;;
            0) break ;;
            *) echo -e "${C_RED}无效选项。${C_RESET}"; sleep 1 ;;
        esac
    done
}
manual_backup() { echo -e "${C_CYAN}=== 手动备份 ===${C_RESET}"; if [ ! -f "${CONFIG_FILE}" ]; then echo -e "${C_YELLOW}请先配置备份 (选项 2)。${C_RESET}"; sleep 2; return; fi; echo -e "${C_YELLOW}正在执行备份...${C_RESET}"; if run_backup; then echo -e "${C_GREEN}手动备份成功！${C_RESET}"; else echo -e "${C_RED}手动备份失败，请检查日志: ${LOG_FILE}${C_RESET}"; fi; read -p "按回车键返回主菜单..."; }

# --- 管理器自身功能 (无变化) ---
update_self() { echo -e "${C_CYAN}=== 检查管理器脚本更新 ===${C_RESET}"; local temp_file=$(mktemp); echo -e "${C_YELLOW}正在下载最新版本...${C_RESET}"; if ! wget -q -O "${temp_file}" "${SCRIPT_URL}"; then echo -e "${C_RED}下载失败。${C_RESET}"; rm -f "${temp_file}"; read -p "按回车返回..."; return; fi; if cmp -s "${SCRIPT_PATH}" "${temp_file}"; then echo -e "${C_GREEN}已是最新版本。${C_RESET}"; rm -f "${temp_file}"; else echo -e "${C_GREEN}发现新版本！正在自动更新...${C_RESET}"; mv "${temp_file}" "${SCRIPT_PATH}"; chmod +x "${SCRIPT_PATH}"; echo -e "${C_GREEN}更新成功！脚本将重启...${C_RESET}"; sleep 2; exec bash "${SCRIPT_PATH}"; fi; read -p "按回车键返回主菜单..."; }
...
# --- 主菜单 (全新重构) ---
main_menu() {
    fetch_remote_versions # 每次循环前都获取一次最新版本
    while true; do
        clear
        ...
        if [ -d "${ST_DIR}" ] && [ -f "${ST_DIR}/package.json" ]; then
            ...
            st_branch=$(cd "${ST_DIR}" && git rev-parse --abbrev-ref HEAD)
        ...
        echo -e "${C_BLUE}=======================================${C_RESET}"
        printf "${C_YELLOW}%-18s ${C_GREEN}%-20s\n" "本地分支:" "$st_branch"
        printf "${C_YELLOW}%-18s ${C_GREEN}%-20s\n" "本地版本:" "$st_version"
        printf "${C_YELLOW}%-18s ${C_CYAN}%-20s\n" "远程 Release:" "$RELEASE_VER"
        ...
        case $choice in
            ...
            2)
               if $is_installed; then
                   if [ -f "${CONFIG_FILE}" ]; then manage_backup; else setup_backup; read -p "按回车键返回。"; fi
               else echo -e "${C_RED}请先安装 SillyTavern (选项0)。${C_RESET}"; sleep 1; fi ;;
            3) if $is_installed; then manual_backup; else echo -e "${C_RED}请先安装 SillyTavern (选项0)。${C_RESET}"; sleep 1; fi ;;
            4) update_self ;;
            5) echo "退出脚本。"; exit 0 ;;
...
case "$1" in
    ...
    *) if [ "$(realpath "$0")" != "${SCRIPT_PATH}" ]; then echo "首次运行，正在自动安装管理器..."; setup_script; else main_menu; fi ;;
esac